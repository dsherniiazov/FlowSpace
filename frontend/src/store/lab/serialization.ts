...
function serializeGraphNode(node: Node): Record<string, unknown> {
  ...
  return {
    ...
    student_tooltip: String(node.data?.studentTooltip ?? ""),
    visual_theme: String(node.data?.visualTheme ?? ""),
    fill_cap: Number(node.data?.fillCap ?? 0),
    decay: 0,
    bias: 0,
...
function deserializeGraphNode(item: Record<string, unknown>, index: number): Node {
  ...
  return {
    ...
    data: {
      ...
      studentTooltip: String(item.student_tooltip ?? item.studentTooltip ?? ""),
      visualTheme: String(item.visual_theme ?? item.visualTheme ?? ""),
      fillCap: Number(item.fill_cap ?? item.fillCap ?? 0),
